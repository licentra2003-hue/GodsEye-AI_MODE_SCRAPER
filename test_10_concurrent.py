    ...
    for i, query in enumerate(QUERIES):
        ...
        try:
            ...
            if resp.status_code in [200, 202]:
                data = resp.json()
                print(f"[OK] [{i+1:02d}/10] Submitted: {query} (Job ID: {data.get('job_id')})")
            else:
                print(f"[ERROR] [{i+1:02d}/10] Failed ({resp.status_code}): {resp.text}")
        except Exception as e:
            print(f"[ERROR] [{i+1:02d}/10] Error submitting: {str(e)}")
