"""
Google AI Mode Scraper API - v1.0
Scrapes Google's AI Mode by clicking the "AI Mode" link
Extracts content from the specific AI Mode HTML structure
"""
...
class ScrapeRequest(BaseModel):
    ...
    query: str = Field(..., description="Search query for Google AI Mode")
    location: str = Field(default="India", description="Location setting")
...
class BrowserManager:
    def __init__(self):
        ...
        self.stealth = None
        self.user_data_dir_base = os.getenv("PLAYWRIGHT_USER_DATA", "playwright_user_data")

    # async def get_or_create_context(self, location: str = "India") -> BrowserContext:
    #     """Get existing context for location or create new one"""
        
    #     if location not in LOCATION_CONFIG:
    #         raise ValueError(f"Invalid location: {location}")
        
    #     if location in self.contexts:
    #         try:
    #             _ = self.contexts[location].pages
    #             print(f"✓ Reusing existing browser context for {location}")
    #             return self.contexts[location]
    #         except:
    #             self.contexts[location] = None

    #     print(f"\n🚀 Initializing browser for location: {location}...")
    ...
    #     user_data_dir = f"{self.user_data_dir_base}_{location.lower().replace(' ', '_')}_aimode"

    #     self.contexts[location] = await self.playwright.chromium.launch_persistent_context(
    ...
    #         extra_http_headers=location_settings['extra_http_headers'],
    #     )
    ...
    async def get_or_create_context(self, location: str = "India") -> BrowserContext:
        ...
        args = [
            ...
            '--start-maximized',
            '--window-size=1920,1080',
            # REMOVED '--disable-gpu' to allow WebGL spoofing to work
        ]

        # Use Chrome's new headless mode if headless is requested
        if headless_flag:
        ...
        self.contexts[location] = await self.playwright.chromium.launch_persistent_context(
            user_data_dir=user_data_dir,
            user_agent=user_agent,
            # Force Playwright's native headless to False so it respects our --headless=new arg
            headless=False, 
            args=args,
            ignore_default_args=['--enable-automation'],
            locale=location_settings['locale'],
            ...
            geolocation=location_settings['geolocation'],
            permissions=location_settings['permissions'],
            # IMPORTANT: Prevent Playwright from overriding window size with default viewport
            no_viewport=True, 
            extra_http_headers=location_settings['extra_http_headers'],
        )
    ...
    async def add_stealth_scripts(self, page: Page):
    ...
    async def stop(self):
        ...
        for location, context in self.contexts.items():
            if context:
                await context.close()
                print(f"✓ Browser closed for {location}")
        self.contexts = {}
...
async def lifespan(app: FastAPI):
    ...
    print("\n" + "="*80)
    print("🤖 Google AI Mode Scraper API v1.0")
    print("="*80)
...
app = FastAPI(
    title="Google AI Mode Scraper API",
    description="Scraper for Google's AI Mode (clicks AI Mode link and extracts response)",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    ...
    return {
        "status": "online",
        "service": "Google AI Mode Scraper API",
        "version": "1.0.0",
        "method": "Clicks 'AI Mode' link on search results"
    }

@app.post("/scrape", response_model=AIModeResult)
async def scrape_ai_mode(request: ScrapeRequest):
    """
    Scrape Google AI Mode for a given query
    
    - Searches Google for the query
    - Clicks "AI Mode" link
    - Waits for response to complete
    - Clicks "Show all related links"
    - Extracts text and sources
    """
    ...
    try:
        print(f"\n{'='*80}")
        print(f"📥 NEW AI MODE REQUEST")
        print(f"Query: {request.query}")
        print(f"Location: {request.location}")
        print(f"{'='*80}")

        context = await browser_manager.get_or_create_context(request.location)
        page = await context.new_page()
        await browser_manager.add_stealth_scripts(page)

        result = await scraper.scrape(page, request.query, request.location)
        
        print(f"\n{'='*80}")
        if result.success:
            print(f"✅ SUCCESS")
            print(f"Text extracted: {len(result.ai_mode_text)} chars")
            print(f"Sources extracted: {len(result.source_links)}")
        else:
...
║     🔗 Clicks "AI Mode" Link Automatically                  ║
║     📚 Extracts Text + Sources                              ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝

🎯 How it works:
   1. Searches Google for your query
   2. Clicks "AI Mode" link on results
   3. Waits for AI response to complete
   4. Clicks "Show all related links"
   5. Extracts structured text and sources
