"""
Google AI Mode Scraper - Worker Version
...
class GoogleAIModeScraper:
    ...
    async def _click_ai_mode_link(self, page: Page) -> bool:
        ...
        try:
            ...
            if "AI Mode" in page_content:
                print("  ✓ 'AI Mode' text found in page source")
            else:
                print("  ✗ 'AI Mode' text NOT found in page source")
                # Save page for debugging
                with open("debug_page_ai_mode.html", "w", encoding="utf-8") as f:
                    f.write(page_content)
                print("  📄 Page saved to debug_page_ai_mode.html")
            ...
            selectors = [
                ...
                'a:has(span:has-text("AI"))',
                'div:has(span:has-text("AI"))',
                'g-menu-item:has-text("AI Mode")',
            ]
            ...
            for i, selector in enumerate(selectors):
                try:
                    ...
                    count = await ai_mode_link.count()
                    if count > 0:
                        print(f"    Found {count} elements with selector: {selector}")
                        if await ai_mode_link.is_visible(timeout=2000):
                            print(f"  ✓ Found visible AI Mode link: {selector}")
                            ...
                            print(f"    Element exists but not visible: {selector}")
                    else:
            ...
            print("  ✗ AI Mode link not found with any selector")
            
            # Last resort: Try to find any element containing "AI Mode"
            try:
                all_elements = await page.locator('*').count()
                print(f"  🔍 Checking all {all_elements} elements for 'AI Mode' text...")
                
                # Use JavaScript to find elements with "AI Mode" text
                found_elements = await page.evaluate("""() => {
                    const elements = [];
                    const allEls = document.querySelectorAll('*');
                    for (const el of allEls) {
                        if (el.textContent && el.textContent.includes('AI Mode')) {
                            elements.push({
                                tag: el.tagName,
                ...
                if found_elements:
                    print(f"  ✓ Found {len(found_elements)} elements containing 'AI Mode':")
                    for elem in found_elements[:5]:  # Show first 5
                        print(f"    - {elem['tag']}: {elem['text'][:50]}... (visible: {elem['visible']})")
                else:
                    print("  ✗ No elements contain 'AI Mode' text")
            except:
                pass
    ...
    async def _human_search(self, page: Page, query: str):
        ...
        try:
            ...
            try:
                ...
                # Wait a bit more for AI Mode link to appear (especially in headless mode)
                await page.wait_for_timeout(5000)  # Wait 5 seconds
                
                # Check if AI Mode link exists
                print("  🔍 Looking for AI Mode link...")
                page_content = await page.content()
                if "AI Mode" in page_content:
                    print("  ✓ 'AI Mode' text found in page")
                else:
                    print("  ✗ 'AI Mode' text NOT found in page")
                    # Save page for debugging
                    with open("debug_page.html", "w", encoding="utf-8") as f:
                        f.write(page_content)
                    print("  📄 Page saved to debug_page.html")
                    
                    # Try to find any AI-related links
                    print("  🔍 Searching for AI-related links...")
                    ai_links = await page.locator('a:has-text("AI")').count()
                    print(f"    Found {ai_links} links with 'AI' text")
                    
                    mode_links = await page.locator('a:has-text("Mode")').count()
                    print(f"    Found {mode_links} links with 'Mode' text")
            except:
